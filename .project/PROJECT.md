...
### THE PROFILE-FIRST FLOW (current) — read this before touching discovery
- **Nothing is looked up until the user has approved a profile.** `run.profileApproved` is the gate; `autoResearch`, `autoCollect` and `collectCompetitorAds` all refuse while it is false, and `AutopilotRunner` no longer auto-starts on mount without it. Collection used to begin the instant a run was created, on searches derived from a homepage nobody had seen — every downstream stage was then correct arithmetic over a list the user never agreed to.
- **Stage 2 is now `/runs/[id]/profile`** (rail: "Your field & rivals", actor `you`, always reachable). Intake redirects new runs straight there, and competitor names on intake are OPTIONAL.
- **The company profile is OFFLINE and deterministic** (`src/lib/admirror/profile.ts` + `src/lib/admirror/profile-build.ts`). `composeProfile()` is the single implementation, called during render on the profile page (no revalidate) and wrapped by the `buildCompanyProfile` action. It reads the site, writes summary/sells/audience/search terms, and proposes rivals ONLY into an empty list — a rebuild never deletes or re-adds curated names, and never overwrites edits marked "Corrected by you."
- **FIELD CLASSIFICATION decides who gets searched** (`src/lib/admirror/category.ts`): 16 categories, each with `field`, `label`, `signals`, `marketTerms` and `neighbours`. `classify()` scores the site's own copy (title/headings ×3); `sweepTerms()` returns the field's market vocabulary, then 2 terms per neighbour, then up to 3 of the company's own words, capped at 8. Stored flat on the run as `dossier.category` with `chosenByYou` — once the user picks a field, a rebuild must never re-read it back to our guess. `readingFrom()`/`plannedTerms()` inflate it; the planned searches are shown on screen BEFORE the lookup runs.
- **Rival lookup = category sweep + per-company profiling** (`src/lib/admirror/rival-scan.ts`, action `scanRivals`). `scanCategoryRivals()` sweeps the field's and neighbours' terms via `readMany`, tags each advertiser with `foundVia` (`category_sweep`/`neighbour_sweep`) and `foundUnder`, then holds every candidate against `relevance.ts`'s market test — rejects are KEPT with reasons and rendered as the "set aside" list. `profileNamedRival()` reads ONE named company's own advertiser page, so a name the user typed never depends on a keyword happening to surface them. Placeholder slots ("Category leader" etc.) are never looked up and are pruned once real advertisers land.
- **Every competitor now carries a profile** (new `competitor` columns: `field`, `categoryLabel`, `categoryRelation`, `positioning`, `foundVia`, `foundUnder`, `adsSeen`, `displayLink`, `reachBand`, `profiledAt`). `positioning` is QUOTED from their own highest-reach ad copy, never paraphrased — a paraphrase of an ad is a claim about their strategy. `reachBand` is the widest band Meta published across their read ads, verbatim; a profiled company with none renders "Reach not published", never a zero.
...
- **Ranking now leads on reach** (`published_reach`, weight 0.28) and an ad with no published figure has that component DROPPED and the rest renormalised — it is never punished for Meta's silence.
- **The competitor list carries reach too** (`src/app/runs/[id]/page.tsx` aggregates, `src/components/run/competitor-map.tsx` renders): a row shows the LARGEST band Meta published across that advertiser's collected ads, verbatim, with a META badge and "N of M ads carry a figure". Bands are deliberately NOT summed — adding two ranges produces a range nobody published, over a sample that is only the ads we read. An advertiser with no published figure gets the sentence, never a zero.
- **Direct advertiser lookup** — the download control on any competitor row calls `collectCompetitorAds` and pulls THAT company's live ads by name. A keyword sweep can miss a named rival whose copy never uses the searched word; this asks the question the user actually has.
- **The missing connection is stated BEFORE a run, not after it** (`src/components/run/reader-status.tsx`). `feedConfigured()` is read in the server components for intake, the run console and the collect screen, and only the boolean crosses to the client. Previously the app admitted the reader wasn't connected only once a sweep had already returned nothing — forty seconds of a progress lamp, then what reads like a failure. The banner is not a blocker: setting up a run, adding known rivals and entering ads by hand all still work and still rank the same way.
- **Unreadable is never reported as empty.** Feed states are `ok · empty · no_key · rate_limited · failed`, each with its own plain-words note printed verbatim; `DiscoveryReport.unreadable` stops a run and says so rather than presenting "0 advertisers" as a finding about the market.
- **The rail shows FIVE user stages, not 15 engine steps** (`src/lib/admirror/stages.ts`): Your site · Who's advertising · Their live ads · The angles · Your ads. State rolls up worst-first so a failure or "your turn" can never hide behind a finished sibling. All 15 internal steps are still recorded and still shown in full on the timeline's "Every step this run took" disclosure.
...
- **Provenance kinds:** "read from the public Ad Library" (LIB badge) — a weaker claim than a human seeing the ad — and "published by Meta" (META badge) for the reach figure only.
...
- **Research console** — five-stage rail with tally lamps; live collection progress, brand read (positioning, ICP, voice, proof shape, plus what the site says and the words searched under) and an editable/prunable competitor map across DIRECT / ADJACENT / ATTENTION.
